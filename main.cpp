...
int Login(TpPessoa &User, char &som);
...
int BuscaBinUsuario(FILE *PtrArq, TpPessoa Usuario);
...
int BSequecialIndPalavra(FILE *PtrArq, TpDicionario Elem);
...
int PosicaoMaiorPessoa(FILE *PtrArq, int qtde)
{
	...
	for (i=1; i<qtde; i++)
	{
		...
		if (stricmp(Maior.Login,BPos.Login) < 0)
		{
 			Maior = BPos;
 			PosMaior = i;
		}
...
int BSequecialIndPalavra(FILE *PtrArq, TpDicionario Elem)
{
	int i=0, TT=TamanhoArquivo(PtrArq);
	TpDicionario Bfr;
	
	fseek(PtrArq, i*sizeof(TpDicionario), SEEK_SET);
	fread(&Bfr, sizeof(TpDicionario), 1, PtrArq);
	while (i<TT && stricmp(Elem.port,Bfr.port) > 0)
	{
		i++;
		fseek(PtrArq, i*sizeof(TpDicionario), SEEK_SET);
		fread(&Bfr, sizeof(TpDicionario), 1, PtrArq);
	}
	if (i<TT && stricmp(Elem.port,Bfr.port) == 0) //achou
		return i;
	else
		return -1;
}
...
int BuscaBinUsuario(FILE *PtrArq, TpPessoa Usuario)
{
	int inicio=0, fim=TamanhoArquivo(PtrArq)/sizeof(TpPessoa)-1, meio;
	TpPessoa Bfr;
	meio = fim/2;

	fseek(PtrArq,meio*sizeof(TpPessoa),SEEK_SET);
	fread(&Bfr,sizeof(TpPessoa),1,PtrArq);
	while (!feof(PtrArq) && inicio<fim && stricmp(Usuario.Login,Bfr.Login) != 0 || Bfr.Status == 1)
	{
 		if (stricmp(Bfr.Login,Usuario.Login) < 0)
 			inicio = meio + 1;
		else fim = meio;
		meio = (inicio+fim)/2;
		fseek(PtrArq,meio*sizeof(TpPessoa),SEEK_SET);
	...
	if (strcmp(Usuario.Login,Bfr.Login) == 0)
		return meio;
	else
		return -1;
}
...
void JogoForca(void)
{
	...
	char letras_utilizadas[30], letra, som;
	FILE *PtrArq;
	int login_status, i, j, encontradas, tent_rest, l_valida, ok, lang;
	
	login_status=Login(User, som);
	PtrArq = fopen("dicionario.dat","rb");
	while(login_status == 1)
	{
		...
		if(lang == 1)
			...
			{
				putsxy("Idioma: Ingles",3,24);
				for(i=0, j=5; i<strlen(Palavra.ing); i++, j+=2)
					putsxy("_",j,18);
				...
				encontradas=0; tent_rest=6;
				while(encontradas<strlen(Palavra.ing) && tent_rest > 0)
				{
					putsxy(letras_utilizadas, 6,7);
					...
					if(l_valida == 0)
					{
						for(i=0, j=5, ok=0; i<strlen(Palavra.ing); i++, j+=2)
						{
							if(letra == toupper(Palavra.ing[i]))
							{
								gotoxy(j,18);
								printf("%c", letra);
								encontradas++;
...
int Login(TpPessoa &User, char &som)
{
	...
	if(pos >= 0)
	{
		...
		{
			textbackground(RED);
			putsxy("Deseja jogar com som?(s/n)",3,10);
			textbackground(cor_fundo);
			fflush(stdin);
			som = toupper(getch());
			fclose(PtrArq);
...
void ConsultarPalavra(FILE *PtrArq)
{
	...
	pos = BSequecialIndPalavra(PtrArq, Reg);
	if(pos >= 0)
	{
		fseek(PtrArq,pos*sizeof(TpDicionario),SEEK_SET);
		fread(&Reg, sizeof(TpDicionario), 1, PtrArq);
...
void ConsultarCadPessoa(FILE *B_Arquivo)
{
	...
	pos = BuscaBinUsuario(B_Arquivo, Reg);
	if(pos >= 0)
	{
		fseek(B_Arquivo,pos*sizeof(TpPessoa),SEEK_SET);
		fread(&Reg, sizeof(TpPessoa), 1, B_Arquivo);
		putsxy("Nome: ",5,7);
		putsxy(Reg.Nome,11,7);
		putsxy("Total de Pontos: ",5,9);
